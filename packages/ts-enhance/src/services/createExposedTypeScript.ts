...
import { readCharactersOfFile } from "./readCharactersOfFile.js";

type ArgumentTypes<TFunction> = TFunction extends (...args: infer TArgs) => any
	? TArgs
	: never;

type ReplaceReturnType<TOriginalType, TReturnType> = (
	...args: ArgumentTypes<TOriginalType>
) => TReturnType;

type Replace<TOriginalType, TReplacements> = {
	[Property in keyof TOriginalType]: Property extends keyof TReplacements
		? TReplacements[Property]
		: TOriginalType[Property];
};

export type ExposedTypeScript = Replace<
	typeof import("typescript"),
	{
		createProgram: ReplaceReturnType<
...
export type ExposedProgram = Replace<
	...
	{
		getTypeChecker: ReplaceReturnType<
			import("typescript").Program["getTypeChecker"],
			ExposedTypeChecker
		>;
	}
> & {
...
export const requireExposedTypeScript = (): ExposedTypeScript => {
	...
	if (originalContentStart.includes("/* TypeStat! */")) {
		previouslyExposedTypeScript = require(
			localRequireFile,
		) as ExposedTypeScript;
		return previouslyExposedTypeScript;
	}
	...
	fs.writeFileSync(
		...
		[
			...
			originalContent.replace(
				"var checker = {",
				"var checker = { /* TypeStat! */ isTypeAssignableTo,",
			),
		].join(EOL),
	);

...
import { convertMapToObject, pluralize } from "typestat-utils";
...
export const createCoreMutationsProvider = (
	...
	return (options: TSEnhanceOptions): Provider | undefined => {
		...
		return () => {
			...
			options.output.stdout(
				chalk.gray(
					`Completed wave ${waveIndex}. Wrote mutations to ${mutationsCount} ${pluralize(
						mutationsCount,
						"file",
					)}.`,
				),
...
const createFileNamesAndServicesCache = (options: TSEnhanceOptions) => {

...
export const fixStrictNonNullAssertionReturnTypes: FileMutator = (request: FileMutationsRequest): readonly IMutation[] =>
...
const isNodeVisitableFunctionLikeDeclaration = (node: ts.Node): node is FunctionLikeDeclarationWithType =>
        ...
        isNodeWithType(node),
    visitFunctionWithBody = (node: FunctionLikeDeclarationWithType, request: FileMutationsRequest): IMutation | undefined => {
        ...
        const returningNull = isTypeFlagSetRecursively(declaredType, ts.TypeFlags.Null),
            returningUndefined = isTypeFlagSetRecursively(declaredType, ts.TypeFlags.Undefined);
        ...
        const missingReturnTypes = (returningNull ? 0 : ts.TypeFlags.Null) | (returningUndefined ? 0 : ts.TypeFlags.Undefined),
            ...
            returningNodeExpressions = collectReturningNodeExpressions(node),
            mutations = collectNonNullMutations(request, node, missingReturnTypes, returningNodeExpressions);

        return mutations.length === 0 ? undefined : combineMutations(...mutations);
    },
    collectNonNullMutations = (

...
export const fixStrictNonNullAssertionCallExpressions: FileMutator = (request: FileMutationsRequest): readonly IMutation[] => {
...
const isVisitableCallExpression = (node: ts.Node): node is ts.CallExpression =>
    ...
    visitCallExpression = (node: ts.CallExpression, request: FileMutationsRequest): IMultipleMutations | undefined => {
    ...
    collectArgumentMutations = (
        request: FileMutationsRequest,
        callingNode: ts.CallExpression,
        functionLikeValueDeclaration: ts.FunctionLikeDeclaration,
    ): readonly IMutation[] => {
        const mutations: IMutation[] = [],
            visitableArguments = Math.min(callingNode.arguments.length, functionLikeValueDeclaration.parameters.length),
        ...
        for (let i = 0; i < visitableArguments; i += 1) {
            const typeOfArgument = typeChecker.getTypeAtLocation(callingNode.arguments[i]),
                typeOfParameter = typeChecker.getTypeAtLocation(functionLikeValueDeclaration.parameters[i]);
    ...
    collectArgumentMutation = (request: FileMutationsRequest, callingArgument: ts.Expression) => {
    ...
    isFunctionBodyOrBlock = (node: ts.Node): node is ts.Block | ts.FunctionLikeDeclaration | ts.SourceFile =>

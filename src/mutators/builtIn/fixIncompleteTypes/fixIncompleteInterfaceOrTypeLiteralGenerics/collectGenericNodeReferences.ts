...
export const expandReferencesForGenericTypes = (
    ...
    for (const referencingNode of referencingNodes) {
        ...
        const relevantNodeWithTypeSignatures = ts.isCallExpression(templatedParentInstantiation)
            ...
            templatedDeclarationSymbol = typeChecker.getTypeAtLocation(relevantNodeWithTypeSignatures).getSymbol();
        ...
        const typeArgumentIndex = templatedParentInstantiation.typeArguments.indexOf(referencingNode),
            templateDeclaredTypeNode = templatedDeclaration.typeParameters[typeArgumentIndex];
...
const collectGenericReferencesOfType = (
        ...
        const genericNodeReferences: ts.Node[] = [],
            ...
            originalType = typeChecker.getTypeAtLocation(interfaceOrTypeLiteral),
            ...
            referencingTypeNodes = request.fileInfoCache.getNodeReferencesAsNodes(templateDeclaredTypeNode);
    ...
    findProvidedTypesForParameter = (
        ...
        const providedNodes: ts.Node[] = [],
            allReferencingNodes = request.fileInfoCache.getNodeReferencesAsNodes(signature);
    ...
    expressionRefersToOriginalType = (
        ...
        const typeChecker = request.services.program.getTypeChecker(),
            expressionNodeType = typeChecker.getTypeAtLocation(potentialCallOrNewExpression);

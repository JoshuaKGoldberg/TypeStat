...
import { initializeSuppressions } from "./suppressions";
...
export const initializeJavaScript = async ({ fileName, project }: InitializeJavaScriptSettings) => {
    ...
    const suppressions = await initializeSuppressions();

    const settings = createJavaScriptConfig({ imports, project, sourceFiles, suppressions, renames });

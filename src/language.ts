 ...
 */
export const createLanguageServices = async (projectPath: string): Promise<LanguageServices> => {
    ...
    const parsedConfiguration = ts.parseJsonConfigFileContent(
        ...
        path.resolve(path.dirname(projectPath)),
        { noEmit: true },
    );

    // Create a basic TypeScript compiler host and program using the parsed compiler options
    const host = ts.createCompilerHost({}, true);
    const program = ts.createProgram(parsedConfiguration.fileNames, compilerConfigOptions, host);

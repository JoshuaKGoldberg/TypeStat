...
const keywordLiteralEquivalents = new Map([
     ...
     */
    invalidExtractedTypeResults = new Set(["", "null", "undefined"]);
...
export const extractRawCodefixUnionTypes = (request: FileMutationsRequest, newText: string): string | undefined => {
    ...
    const processTypeNode = (type: ts.TypeNode): string | undefined => {
            ...
            const textForNode = getTextForNode(type),
                textAlias = findAliasOfType(textForNode, request.options.types.aliases);
        ...
        processUnionTypeNode = (type: ts.UnionTypeNode): string | undefined => {
            const processedTypes = type.types.map(processTypeNode),
                filteredProcessedTypes = processedTypes.filter(isNotUndefined);
        ...
        getTextForNode = (type: ts.TypeNode): string => {
        ...
        sourceText = `type _ = ${newText.substring(": ".length)}`,
        sourceFile = ts.createSourceFile("_.tsx", sourceText, ts.ScriptTarget.Latest),
        typeResult = processTypeNode((sourceFile.statements[0] as ts.TypeAliasDeclaration).type);
